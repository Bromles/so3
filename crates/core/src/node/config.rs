...
impl NodeConfig {
    ...
    pub fn from_env_with(get_var: impl Fn(&str) -> Option<String>) -> So3Result<Self> {
        Self::from_file_and_env(None, get_var)
    }

    /// # Errors
    ///
    /// Returns an error when the optional TOML file or any supported environment value
    /// cannot be parsed. Environment variables override TOML values.
    pub fn load() -> So3Result<Self> {
    ...
    /// cannot be parsed.
    pub fn load_with(get_var: impl Fn(&str) -> Option<String>) -> So3Result<Self> {
        let file_config = load_optional_config_file(&get_var)?;
        Self::from_file_and_env(file_config.as_ref(), get_var)
    }

    /// # Errors
    ///
    /// Returns an error when the TOML file does not exist or cannot be parsed.
    pub fn from_toml_file(path: impl AsRef<Path>) -> So3Result<Self> {
        let file_config = parse_config_file(path)?;
        Self::from_file_and_env(Some(&file_config), |_| None)
    }

    /// # Errors
    ///
    /// Returns an error when the TOML payload cannot be parsed.
    pub fn from_toml_str(value: &str) -> So3Result<Self> {
        let file_config = parse_toml_config(value)?;
        Self::from_file_and_env(Some(&file_config), |_| None)
    }
...
mod tests {
    ...
    #[test]
    fn from_toml_str_parses_config_file() {
        let config = NodeConfig::from_toml_str(TOML_CONFIG).unwrap();
        config.validate().unwrap();
    ...
    #[test]
    fn load_with_prefers_env_over_toml_values() {
        let file_config = super::parse_toml_config(TOML_CONFIG).unwrap();
        let config = NodeConfig::from_file_and_env(Some(&file_config), |name| match name {
            super::OBJECT_ADDR_ENV => Some(OVERRIDE_OBJECT_ADDR.to_owned()),
            super::OBJECT_REQUEST_TIMEOUT_SECS_ENV => Some(OVERRIDE_TIMEOUT_SECS.to_string()),
            _ => None,
        })
    ...
    #[test]
    fn from_toml_file_reads_config_from_disk() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("so3.toml");
        std::fs::write(&config_path, TOML_CONFIG).unwrap();

        let config = NodeConfig::from_toml_file(&config_path).unwrap();
