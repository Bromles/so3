...
use crate::primitive::unseekables::UnseekablesKind::{FullKeyRoute, FullRangeRoute, RoutingKeys, RoutingRanges};
...
pub trait Unseekables<K: Unseekable>: Routables<K> + Iterator {
    ...
    fn intersecting_seekables<T: Seekable>(&self, intersecting: impl Seekables<T>) -> impl Unseekables<K>;
    fn intersecting_seekables_with_slice<T: Seekable>(&self, intersecting: impl Seekables<T>, slice: Slice) -> impl Unseekables<K>;
    fn intersecting_unseekables<T: Unseekable>(&self, intersecting: impl Unseekables<T>) -> impl Unseekables<K>;
    fn intersecting_unseekables_with_slice<T: Unseekable>(&self, intersecting: impl Unseekables<T>, slice: Slice) -> impl Unseekables<K>;
    
    fn without_ranges(&self, ranges: Ranges) -> impl Unseekables<K>;
    fn without_unseekables<T: Unseekable>(&self, subtract: impl Unseekables<T>) -> impl Unseekables<K>;
