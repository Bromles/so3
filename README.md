...
- Maelstrom-адаптер для проверки части поведения через `lin-kv` и `g-set` сценарии Jepsen Maelstrom.
...
| `so3-maelstrom` | Адаптер stdin/stdout для Jepsen Maelstrom `lin-kv` и `g-set` тестов      |
