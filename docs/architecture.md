...
Проект является экспериментальным proof of concept распределенного объектного хранилища с
репликацией на основе консенсуса без постоянного лидера.

Архитектура оценивается не как готовая замена промышленным S3-совместимым системам, а как проверка
исследовательской гипотезы: Accord-подобный безлидерный протокол может быть применим к
metadata/control plane объектного хранилища, если прототип сохраняет корректность, переживает
допустимые отказы, предсказуемо деградирует, локализует конфликты и не имеет постоянного leader
bottleneck.
...
sequenceDiagram
    ...
    participant MP as Metadata-запросы к узлам
    participant B as Локальный blob repo
    participant BP as Blob-узлы
    C ->> A: GET /bucket/key
    A ->> O: read(key)
    ...
    QM -->> O: метаданные с максимальной версией
...
Чтения (`GET`, `HEAD`) не проходят через координатор консенсуса. Вместо этого используется quorum read
метаданных: локальные метаданные запрашиваются вместе с метаданными всех узлов, выбирается результат
с наибольшей версией. Если blob отсутствует локально, он восстанавливается с одного из узлов.
...
6. Иначе записывает и отправляет `Accept` параллельно с тем же quorum-driven early exit, объединяя зависимости из ответов.
7. Записывает `Committed`, отправляет `Commit` параллельно до ответа кворума, применяет команду локально через
   общий `ApplyEngine` с reorder buffer и отправляет `Apply` другим узлам.

Blob-репликация также выполняется параллельно: blob отправляется всем узлам одновременно, и выполнение
продолжается при достижении кворума подтверждённых записей.
...
flowchart LR
    ...
    runtime --> service["MaelstromService\n(каждый узел координирует локально)"]
...
- каждый Maelstrom-узел координирует клиентские запросы локально (production-бинарь `so3` тоже позволяет
  любому узлу координировать запросы, пришедшие на его S3-подобный API);
...
Основные риски, которые остаются за рамками текущего прототипа и должны учитываться при интерпретации PoC:
...
- blob RPC-вызвы не имеют дедлайнов на отдельные операции;
- производительность не оптимизировалась и не является самостоятельной целью; нагрузочные сценарии нужны прежде
  всего для анализа динамики под отказами, конфликтами и восстановлением.

Полный список аудита, TODO и пробелов в тестировании находится в [../agents.md](../agents.md).
Методология исследовательской проверки находится в [research-plan.md](research-plan.md).
