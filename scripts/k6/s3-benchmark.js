 ...
 *
 * Uses the official k6 AWS S3Client (Grafana jslib). Env: SO3_ADDR, SO3_BUCKET,
 * OBJECT_SIZE, VUS, DURATION, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY.
 *
 * Single run:  k6 run scripts/k6/s3-benchmark.js
 * Multi-run:   python scripts/k6/run-backend-benchmark.py --backend so3 --runs 30
 */
...
export const options = {
  summaryTrendStats: [
    'avg', 'med', 'p(10)', 'p(25)', 'p(75)', 'p(90)', 'p(95)', 'p(99)', 'min', 'max'
  ],
...
const headMs = new Trend('s3_head_ms', true)
...
export default async function () {
  ...
  try {
    ...
    check(obj, {
      ...
      // jslib reads ETag case-sensitively; server may send lowercase "etag".
  ...
  // headObject isn't in k6 jslib S3Client — use plain HTTP HEAD.
  t = Date.now()
  {
    const headUrl = `${BASE_URL}/${BUCKET}/${key}`
...
/**
 * IQR-based std-dev estimation for a Trend metric.
 * σ ≈ IQR / 1.3490  (robust against outliers, no raw samples needed).
 */
