...
const REQUIRE_VERSION_ID = (__ENV.REQUIRE_VERSION_ID || "1") === "1";
const SIGN_HEAD = (__ENV.SIGN_HEAD || "0") === "1";
...
const headMs = new Trend("s3_head_ms", true); // signed/unsigned http HEAD (not in jslib S3Client)
...
export default async function () {
    ...
    try {
        ...
        if ((__ENV.DEBUG_ERRORS || "0") === "1") {
            console.error(`PUT failed for ${key}: ${e && e.message ? e.message : e}`);
        }
    ...
    // ── HEAD (metadata-only via plain or signed http) ──────────────────────────
    // headObject is not part of the k6 jslib S3Client API. For so3 we keep the
    // original unsigned HEAD path; MinIO/Garage need SigV4 for private buckets.
    t = Date.now();
    {
        let headUrl = `${BASE_URL}/${BUCKET}/${key}`;
        const headParams = {tags: {operation: "head"}};

        if (SIGN_HEAD) {
            const signedHead = s3.signature.sign(
                {
                    method: "HEAD",
                    endpoint: s3.endpoint,
                    path: encodeURI(`/${BUCKET}/${key}`),
                    headers: {Host: s3.endpoint.host},
                },
                {},
            );
            headUrl = signedHead.url;
            headParams.headers = signedHead.headers;
        }

        const res = http.head(headUrl, headParams);
        ...
        const ok = check(res, {
            ...
            "head: x-amz-version-id present when required": (r) =>
                !REQUIRE_VERSION_ID || r.headers["X-Amz-Version-Id"] !== undefined,
        });
        s3Errors.add(!ok);
    }
 ...
 * with scripts/k6/run-backend-benchmark.py.
 */
...
export function handleSummary(data) {
    ...
    const lines = [
        ...
        "  For cross-run statistics: python scripts/k6/run-backend-benchmark.py --backend so3 --runs 30",
        "",
    ];
